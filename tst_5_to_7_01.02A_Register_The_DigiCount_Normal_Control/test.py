...
def main():
    ...
    try:
        test.log("Register the DigiCount Normal Control")
        ...
        #NOTE: You don't need to confirm that every Range value is correct, spot check a few throughout the list of parameters"
        test.log("Step #" + str(step_counter)); step_counter += 1        
        shared.newdigicountcontroldialog.selectLevelComboboxItem("Normal DigiCount™ control")
        shared.newdigicountcontroldialog.setLotNumber("S1401001N")
        shared.newdigicountcontroldialog.enterDigiCountTableData(getattr(shared.config,"expected_csv_result_dir") + "/digicountnormal.csv")
        ...
        #shared.newdigicountcontroldialog.digiCountTableDataToCSV(getattr(shared.config,"expected_csv_result_dir") + "/digicountnormal_output.csv") 
        shared.newdigicountcontroldialog.setExpirationDate()
        #Confirm the lot number from the assay sheet appears in the "Lot" entry field
        shared.newdigicountcontroldialog.confirmLotNumber("S1401001N")
        #Confirm the Level combobox defaults to "Normal"
        shared.newdigicountcontroldialog.confirmLevelComboBoxSelectedItem("Normal DigiCount™ control")
        #Confirm that the value in the "Analyzer" combobox matches the Analyzer that you are creating the control on
        ...
        #Confirm that all the data from the assay sheet is inserted with a single 2D scan        
        shared.newdigicountcontroldialog.confirmDigiCountTableData(getattr(shared.config,"expected_csv_result_dir") + "/digicountnormal.csv")
        #Confirm that the "Historical SD" column already has entries for all the rows
        shared.newdigicountcontroldialog.setExpirationDate()
        ...
        #Confirm the DigiCount™ Normal sample is registered
        shared.qctab.confirmRegisteredNormalLabel()
        #Confirm the barcode is now displayed in the Control in the toolbar
        shared.qctab.confirmBarcode("S1401001N")        
        #Confirm that upon leaving the "Activate/Edit DigiCount™ Controls" dialog you are now in the QC > DigiCount™ page NOT the QC > Overview page
